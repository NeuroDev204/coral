...
CoralNotification::CoralNotification()
{
...
void CoralNotification::setMessage(const String& message, const std::pair<String, String>& link, bool autohide)
{
	...
	brand_label_.setText("Coral", NotificationType::dontSendNotification);
	brand_label_.setFont(theme.getTitleFont().withHeight(16.0f));
	brand_label_.setColour(Label::textColourId, Colour(FXCOLOR(HighlightedFill)).withAlpha(1.0f));
	brand_label_.setJustificationType(Justification::centredLeft);
	brand_label_.setInterceptsMouseClicks(false, false);
	brand_label_.setBounds(42, 6, 90, 20);
	addAndMakeVisible(brand_label_);
...
void CoralNotification::showMessage(bool autohide)
{
	int last_line = 0;
    int y = 0;
    ...
    {
        brand_label_.setVisible(false);
        Desktop::getInstance().getAnimator().fadeIn(this, 200);
        setVisible(true);
    }
}

void CoralNotification::paint(Graphics& g)
{
	DropShadow shadow;
	Path path;
    ...
    CoralTheme::paintGlassCard(g, getLocalBounds().toFloat(), 16.0f);
	CoralTheme::paintCoralMark(g, { 16.0f, 8.0f, 22.0f, 16.0f }, Colour(FXCOLOR(HighlightedFill)).withAlpha(1.0f));    
}

void CoralNotification::timerCallback()
{
	stopTimer();
	setVisible(false);
	removeFromDesktop();
