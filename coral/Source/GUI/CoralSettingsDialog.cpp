...
//==============================================================================
CoralSettingsDialog::CoralSettingsDialog() : CoralWindow("Settings"), tooltip_window_(this)
{
...
void CoralSettingsDialog::closeButtonPressed()
{
	exitModalState(0);
	removeFromDesktop();
}

void CoralSettingsDialog::paint(Graphics& g)
{
	CoralWindow::paint(g);
...
void CoralSettingsDialog::SettingsButton::paint(Graphics& g)
{
...
bool CoralSettingsDialog::keyPressed(const KeyPress& key)
{
...
CoralSettingsDialog::SettingsComponent::SettingsComponent()
{
...
void CoralSettingsDialog::SettingsComponent::resized()
{
...
void  CoralSettingsDialog::SettingsComponent::buttonClicked(Button* button)
{
...
CoralSettingsDialog::SettingsPane::SettingsPane(String name)
{
	name_ = name;
	auto& theme = dynamic_cast<CoralTheme&>(getLookAndFeel());
...
void CoralSettingsDialog::SettingsPane::paint(Graphics&)
{
    auto& theme = dynamic_cast<CoralTheme&>(getLookAndFeel());
...
CoralSettingsDialog::AudioSettingsPane::AudioSettingsPane() :
	SettingsPane("Audio"),
	...
	reset_presets_button_(TRANS("Reset presets to factory defaults"))
{
	...
	prioritize_new_output_toggle_.setToggleState(CoralController::getInstance().isNewOutputPrioritized(), NotificationType::dontSendNotification);
	prioritize_new_output_toggle_.onClick = [this]() { CoralController::getInstance().setNewOutputPrioritized(prioritize_new_output_toggle_.getToggleState()); };

	auto preset_modified = false;
	auto preset_count = CoralModel::getModel().getPresetCount();
	for (auto i = 0; i < preset_count; i++)
	{
		if (CoralModel::getModel().isPresetModified(i))
		{
			preset_modified = true;
			break;
		}
	}
	reset_presets_button_.setEnabled(CoralModel::getModel().getUserPresetCount() > 0 || preset_modified);
...
void CoralSettingsDialog::AudioSettingsPane::resized()
{
...
void CoralSettingsDialog::AudioSettingsPane::paint(Graphics& g)
{
...
void CoralSettingsDialog::AudioSettingsPane::setText()
{
...
void CoralSettingsDialog::AudioSettingsPane::resizeResetButton(int x, int y)
{
	...
	int buttonWidth = std::min(reset_presets_button_.getBestWidthForHeight(BUTTON_HEIGHT * lineCount), MAX_BUTTON_WIDTH);
...
void CoralSettingsDialog::AudioSettingsPane::visibilityChanged()
{
	if (isVisible())
	{
...
void CoralSettingsDialog::AudioSettingsPane::mouseEnter(const MouseEvent& mouse_event)
{
	Component::mouseEnter(mouse_event);
}

void CoralSettingsDialog::AudioSettingsPane::mouseExit(const MouseEvent& mouse_event)
{
	Component::mouseEnter(mouse_event);
}

CoralSettingsDialog::GeneralSettingsPane::GeneralSettingsPane() :
...
{
	StringArray hotKeySettingsKeys = { CoralController::HK_CMD_ON_OFF, CoralController::HK_CMD_OPEN_CLOSE, CoralController::HK_CMD_NEXT_PRESET, CoralController::HK_CMD_PREVIOUS_PRESET, CoralController::HK_CMD_NEXT_OUTPUT };
	StringArray hotkey_names = { "Turn Coral On/Off", "Open/Close Coral",
	...
	bool hotkey_enabled = CoralModel::getModel().getHotkeySupport();
	for (int i=0; i<hotkey_names.size(); i++)
	{
		auto label = new CoralHotkeyLabel(hotkey_names[i], hotKeySettingsKeys[i]);
    ...
    {
        hotkeys_toggle_.setToggleState(!CoralModel::getModel().getHotkeySupport(), NotificationType::dontSendNotification);
    }
    else
    {
	...
	hotkeys_toggle_.onClick = [this]()  { 
											CoralController::getInstance().enableHotkeys(!hotkeys_toggle_.getToggleState());
											bool enabled = CoralModel::getModel().getHotkeySupport();
	...
	launch_toggle_.setToggleState(CoralController::getInstance().isLaunchOnStartup(), NotificationType::dontSendNotification);
	launch_toggle_.onClick = [this]() { CoralController::getInstance().setLaunchOnStartup(launch_toggle_.getToggleState()); };

    hide_help_tips_toggle_.setToggleState(CoralController::getInstance().isHelpTooltipsHidden(), NotificationType::dontSendNotification);
    hide_help_tips_toggle_.onClick = [this]() { CoralController::getInstance().setHelpTooltipsHidden(hide_help_tips_toggle_.getToggleState()); };
	
	hide_notifications_toggle_.setToggleState(CoralController::getInstance().isNotificationsHidden(), NotificationType::dontSendNotification);
	hide_notifications_toggle_.onClick = [this]() { CoralController::getInstance().setNotificationsHidden(hide_notifications_toggle_.getToggleState()); };
...
void CoralSettingsDialog::GeneralSettingsPane::resized()
{
	auto bounds = getLocalBounds().withLeft(X_MARGIN).withTop(Y_MARGIN).withHeight(TITLE_HEIGHT);
	title_.setBounds(bounds);

    language_switch_.setBounds(X_MARGIN, LANGUAGE_SWITCH_Y, CoralLanguage::WIDTH, CoralLanguage::HEIGHT);
...
void CoralSettingsDialog::GeneralSettingsPane::paint(Graphics& g)
{
	g.fillAll(getLookAndFeel().findColour(ResizableWindow::backgroundColourId));
...
void CoralSettingsDialog::GeneralSettingsPane::setText()
{
    auto& theme = dynamic_cast<CoralTheme&>(LookAndFeel::getDefaultLookAndFeel());

    int height = CoralSettingsDialog::SettingsComponent::HEIGHT;
...
CoralSettingsDialog::HelpSettingsPane::HelpSettingsPane() : SettingsPane("Help"), auto_updates_toggle_(TRANS("Automatic updates"))
{	
	...
	auto_updates_toggle_.setToggleState(CoralController::getInstance().getAutoUpdates(), NotificationType::dontSendNotification);
	...
	auto_updates_toggle_.onClick = [this]() {
		CoralController::getInstance().setAutoUpdates(auto_updates_toggle_.getToggleState());
	};
...
void CoralSettingsDialog::HelpSettingsPane::resized()
{
	auto bounds = getLocalBounds().withLeft(X_MARGIN).withTop(Y_MARGIN).withHeight(TITLE_HEIGHT);
	title_.setBounds(bounds);
...
void CoralSettingsDialog::HelpSettingsPane::paint(Graphics& g)
{
	g.fillAll(getLookAndFeel().findColour(ResizableWindow::backgroundColourId));
...
void CoralSettingsDialog::HelpSettingsPane::setText()
{
    auto& theme = dynamic_cast<CoralTheme&>(LookAndFeel::getDefaultLookAndFeel());
