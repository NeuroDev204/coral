...
#include "CoralPresetNameEditor.h"

class CoralPresetMenuItem : public PopupMenu::CustomComponent, private TextEditor::Listener
{
public:
	enum class Status { Empty = 0, Valid, Invalid };
	enum class Action { Save = 1, Rename };

	CoralPresetMenuItem(Action action) : PopupMenu::CustomComponent(false)
	{
		...
		preset_editor_.onReturnKey = [this, action]() {
			if (preset_status_ == Status::Valid)
			{
				if (action == Action::Save)
				{
					CoralController::getInstance().savePreset(preset_name_);
					triggerMenuItem();
				}
				else
				{
					CoralController::getInstance().renamePreset(preset_name_);
					triggerMenuItem();
				}
			}
		};
	}
	~CoralPresetMenuItem() = default;
...
private:
	...
	JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(CoralPresetMenuItem)
};

//==============================================================================
CoralMainWindow::CoralMainWindow() : CoralWindow("Coral", true), power_button_(L"powerButton"), resize_button_(L"resizeButton", DrawableButton::ButtonStyle::ImageFitted), donate_button_(L"donateButton", DrawableButton::ButtonStyle::ImageFitted), minimize_button_(L"minimizeButton", DrawableButton::ButtonStyle::ImageFitted)
{
	setName("Coral");
	setOpaque(false);
    enableShadow(false);
	...
	power_button_.setPowerState(CoralModel::getModel().getPowerState());
...
void CoralMainWindow::show()
{
	...
	int x = 0;
	int y = 0;
	CoralController::getInstance().getWindowPosition(x, y);
	if (x != 0 || y != 0)
...
void CoralMainWindow::showLiteView()
{
	setContent(&lite_view_);
    setResizeImage();
	view_switcher_.syncFromController();
	setAlwaysOnTop(CoralController::getInstance().isAlwaysOnTop());
	lite_view_.startVisualizer();
}

void CoralMainWindow::showProView()
{
    pro_view_.update();
...
void CoralMainWindow::update()
{
	pro_view_.update();
}

void CoralMainWindow::startVisualizer()
{
    pro_view_.startVisualizer();
    lite_view_.startVisualizer();
}

void CoralMainWindow::pauseVisualizer()
{
    pro_view_.pauseVisualizer();
    lite_view_.pauseVisualizer();
}

void CoralMainWindow::setLookAndFeel()
{
...
void CoralMainWindow::setResizeImage()
{
	if (CoralController::getInstance().getCurrentView() == ViewType::Pro)
	{
...
void CoralMainWindow::setIcon(bool power, bool processing)
{
#if defined(_WIN32)
	...
	if (power)
	{
		if (processing)
		{
			if (CoralTheme::getThemeMode() == CoralThemeMode::Dark)
...
void CoralMainWindow::applyDockIcon()
{
	auto* peer = getPeer();
	if (peer == nullptr)
		return;
...
void CoralMainWindow::enablePowerButton(bool enable)
{
...
bool CoralMainWindow::keyPressed(const KeyPress& key)
{
	if (key.getModifiers().isAltDown() && key.getKeyCode() == juce::KeyPress::returnKey)
	{
		CoralController::getInstance().setMenuClicked(true);
		showMenu();
		return true;
	}
...
void CoralMainWindow::visibilityChanged()
{
	auto* peer = getPeer();
	if (peer)
	{
...
void CoralMainWindow::showMenu()
{
	auto settingsClicked = []() {
		CoralSettingsDialog settings_dialog;
		settings_dialog.runModalLoop();
		CoralController::getInstance().refreshOutputList();
	};

	PopupMenu popup_menu;
	popup_menu.addItem(TRANS("Settings"), settingsClicked);
	popup_menu.addSeparator();
	popup_menu.addItem(TRANS("Exit"), []() { CoralController::getInstance().exit(); });
	popup_menu.showAt(&menu_button_);
}

void CoralMainWindow::buttonClicked(Button* button)
{
	if (button == &power_button_)
	{
		auto power_state = !CoralModel::getModel().getPowerState();

		CoralController::getInstance().setPowerState(power_state);
	...
	{
		CoralController::getInstance().switchView();
		setResizeImage();
	}
...
#else
		CoralController::getInstance().hideMainWindow();
#endif
	}
}

void CoralMainWindow::mouseEnter(const MouseEvent&)
{
	if (menu_button_.isMouseOver(true) && !CoralModel::getModel().isMenuClicked())
	{
		auto& theme = dynamic_cast<CoralTheme&>(getLookAndFeel());
...
void CoralMainWindow::modelChanged(CoralModel::Event)
{
	power_button_.setPowerState(CoralModel::getModel().getPowerState());
}

void CoralMainWindow::userTriedToCloseWindow()
{
	CoralController::getInstance().hideMainWindow();
}

void CoralMainWindow::closeButtonPressed()
{
	CoralController::getInstance().hideMainWindow();
}

void CoralMainWindow::moved()
{
	auto bounds = getBounds();
	auto desktop_bounds = Desktop::getInstance().getDisplays().getTotalBounds(true);
	if (desktop_bounds.contains(bounds))
		CoralController::getInstance().saveWindowPosition(bounds.getX(), bounds.getY());
}

void CoralMainWindow::lookAndFeelChanged()
{
	setLookAndFeel();
	view_switcher_.syncFromController();
	repaint();
...
CoralMainWindow::ViewSwitcher::ViewSwitcher()
{
...
void CoralMainWindow::ViewSwitcher::syncFromController()
{
	const bool pro = CoralController::getInstance().getCurrentView() == ViewType::Pro;
	simple_.setToggleState(!pro, dontSendNotification);
	pro_.setToggleState(pro, dontSendNotification);
}

void CoralMainWindow::ViewSwitcher::resized()
{
...
void CoralMainWindow::ViewSwitcher::paint(Graphics& g)
{
...
void CoralMainWindow::ViewSwitcher::buttonClicked(Button* button)
{
