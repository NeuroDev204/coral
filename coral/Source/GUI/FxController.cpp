...
void FxController::initPresets()
{
	...
	if (factsoft_dir.isDirectory())
	{
		for (int i = 0; i < kNumStandardPresetFiles; ++i)
		{
			...
			FxModel::Preset preset;
			preset.name = preset_name;
			preset.type = FxModel::PresetType::AppPreset;
	...
	if (user_presets_dir.isDirectory())
	{
		...
		for (const auto& preset_file : user_preset_paths)
		{
			...
			FxModel::Preset preset;
			preset.name = preset_name;
			preset.type = FxModel::PresetType::UserPreset;
			preset.modified = false;
			preset.path = preset_file.getFullPathName();
			presets.add(preset);
		}
...
void FxController::hideMainWindow()
{
	...
	if (minimize_tip_)
	{
		...
		Timer::callAfterDelay(2000, []() {
			FxModel::getModel().pushMessage(TRANS("Coral is in the system tray\r\nClick the Coral icon to reopen"));
			});
	}
}
	...
	case WM_HOTKEY:
	{
		if (w_param == CMD_ON_OFF)
		{
			if (!SysInfo::isRemoteSession())
			{
				...
				FxModel::getModel().pushMessage(controller->FormatString(TRANS("Coral is %s."), param));
			}
		}
		...
		if (w_param == CMD_NEXT_PRESET)
		{
			if (!SysInfo::isRemoteSession())
			{
				int preset_index = FxModel::getModel().getCurrentPresetIndex();
				preset_index++;

				if (preset_index > FxModel::getModel().getPresets().size() - 1)
				{
					preset_index = 0;
				}

				controller->setPreset(preset_index, true);
				FxModel::getModel().pushMessage(controller->FormatString(TRANS("Preset: %s"), FxModel::getModel().getCurrentPresetName()));
			}
		}
		if (w_param == CMD_PREVIOUS_PRESET)
		{
			if (!SysInfo::isRemoteSession())
			{
				...
				if (preset_index < 0)
				{
					preset_index = FxModel::getModel().getPresets().size() - 1;
				}
		...
		if (w_param == CMD_NEXT_OUTPUT)
		{
			if (!SysInfo::isRemoteSession())
			{
				int current_output_index = FxModel::getModel().getCurrentOutputIndex();
				if (current_output_index >= 0)
				{
					auto active_devices = FxModel::getModel().getOutputs();
					int i = 0;
					...
					if (new_output_index != -1)
					{
						...
						FxModel::getModel().pushMessage(controller->FormatString(TRANS("Playback: %s"), FxModel::getModel().getCurrentOutputName()));
					}
				}
			}
		}
