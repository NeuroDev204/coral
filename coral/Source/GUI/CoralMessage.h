...
/*
*/
class CoralMessage : public CoralWindow
{
public:
	CoralMessage(String message, const std::pair<String, String>& link);
	~CoralMessage() = default;
...
private:
	...
	JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(CoralMessage)
};

class CoralConfirmationMessage : public CoralWindow
{
public:
    enum Style { YesNo = 1, OK };

    CoralConfirmationMessage(String message, Style style, int x, int y) : CoralWindow(), message_content_(message, style)
    {
    ...
    ~CoralConfirmationMessage() = default;

    CoralConfirmationMessage() = delete;
    ...
    {
        // Construct the CoralConfirmationMessage object with the provided values.
        CoralConfirmationMessage confirmation_message(message, style, x, y);
...
private:
    ...
    public:
        MessageComponent(String message, Style style) : yes_button_(TRANS("Yes")), no_button_(TRANS("No")), ok_button_(TRANS("OK"))
        {
            ...
            if (style == CoralConfirmationMessage::Style::YesNo)
            {
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(CoralConfirmationMessage)
};
#endif
