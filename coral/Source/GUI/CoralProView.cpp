...
//==============================================================================
CoralProView::CoralProView() : tool_tip_(this)
{
...
void CoralProView::startVisualizer()
{
	visualizer_.start();
}

void CoralProView::pauseVisualizer()
{
	visualizer_.pause();
}

void CoralProView::update()
{
...
void CoralProView::setLookAndFeel()
{
	audio_controls_.setLookAndFeel();
}

void CoralProView::resized()
{
...
void CoralProView::paint(Graphics& g)
{
	auto area = getLocalBounds().reduced(22, 16);
	...
	auto sidebar = area.removeFromRight(SIDEBAR_WIDTH);
	area.removeFromRight(14);
	const float r = (float) CoralTheme::CARD_CORNER_RADIUS;

	CoralTheme::paintGlassCard(g, area.toFloat(), r);
	CoralTheme::paintGlassCard(g, sidebar.toFloat(), r);
	CoralTheme::paintGlassCard(g, vis.toFloat(), r);

	auto enable_controls = CoralModel::getModel().getPowerState();
...
void CoralProView::comboBoxChanged(ComboBox* combobox)
{
	CoralView::comboBoxChanged(combobox);
}

void CoralProView::modelChanged(CoralModel::Event model_event)
{
	CoralView::modelChanged(model_event);
...
void CoralProView::mouseEnter(const MouseEvent& mouse_event)
{
	CoralView::mouseEnter(mouse_event);
}

void CoralProView::mouseExit(const MouseEvent& mouse_event)
{
	CoralView::mouseEnter(mouse_event);
}