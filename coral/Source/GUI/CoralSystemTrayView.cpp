...
const GUID CoralSystemTrayView::trayIconGuid_ =
{ 0xa8e96325, 0x5269, 0x443c, { 0xa0, 0xd8, 0xd, 0x2, 0x56, 0x2f, 0xe5, 0x53 } };


CoralSystemTrayView::CoralSystemTrayView()
{
...
void CoralSystemTrayView::modelChanged(CoralModel::Event model_event)
{
    if (!CoralController::getInstance().isNotificationsHidden() && model_event == CoralModel::Event::Notification)
    {
        showNotification();
    }
}

void CoralSystemTrayView::setStatus(bool power, bool processing)
{
...
Point<int> CoralSystemTrayView::getSystemTrayWindowPosition(int width, int height)
{
    Point<int> pos = { 0, 0 };
...
void CoralSystemTrayView::addIcon()
{
    NOTIFYICONDATA nid = { sizeof(nid) };
    ...
    {
        if (CoralController::getInstance().isAudioProcessing())
        {
            if (CoralTheme::getThemeMode() == CoralThemeMode::Dark)
            {
                nid.hIcon = LoadIcon(hInst, L"IDI_LOGO_RED");
...
void CoralSystemTrayView::showContextMenu()
{
    ...
    for (auto i = 0; i < count; i++)
    {
        ...
        menu_item.setID(id);
        if (id - PRESET_MENU_ID_START == CoralModel::getModel().getSelectedPreset())
        {
        ...
        auto handler = [preset = id]() {
            CoralController::getInstance().setPreset(preset - PRESET_MENU_ID_START);
        };
    ...
    context_menu.addItem(MENU_ID_OPEN, TRANS("Open Coral"),
        !CoralController::getInstance().isWindowVisible(), false);
    context_menu.addItem(MENU_ID_POWER,
        CoralModel::getModel().getPowerState() ? TRANS("Power off") : TRANS("Power on"));
    ...
    theme_menu.addItem(1001, TRANS("Dark"), true,
        CoralTheme::getThemeMode() == CoralThemeMode::Dark, []() { CoralTheme::setThemeMode(CoralThemeMode::Dark); });
    theme_menu.addItem(1002, TRANS("Light"), true,
        CoralTheme::getThemeMode() == CoralThemeMode::Light, []() { CoralTheme::setThemeMode(CoralThemeMode::Light); });
    theme_menu.addItem(1003, TRANS("System default"), true,
        CoralTheme::getThemeMode() == CoralThemeMode::SystemDefault,
        []() { CoralTheme::setThemeMode(CoralThemeMode::SystemDefault); });
...
void CoralSystemTrayView::addOutputDeviceMenu(PopupMenu* context_menu)
{
    // Linux: simplified - just show the default output
    auto selected = CoralModel::getModel().getSelectedOutput();
...
void CoralSystemTrayView::showNotification()
{
...
String CoralSystemTrayView::getTruncatedText(const String& text, int max_length)
{
    if (text.length() <= max_length) return text;
    return text.substring(0, max_length - 3) + "...";
}

LRESULT CALLBACK CoralSystemTrayView::wndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    if (message == WMAPP_FXTRAYICON)
    {
        if (LOWORD(lParam) == WM_RBUTTONUP)
        {
            CoralSystemTrayView* view =
                reinterpret_cast<CoralSystemTrayView*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
            if (view != nullptr)
            {
        ...
        else if (LOWORD(lParam) == WM_LBUTTONDBLCLK)
        {
            ...
            if (view != nullptr)
            {
                CoralController::getInstance().showMainWindow();
            }
        }
    }
        ...
        if (view != nullptr)
        {
            switch (LOWORD(wParam))
            {
                case MENU_ID_OPEN:
                    ...
                    break;
                case MENU_ID_POWER:
                    CoralController::getInstance().powerOn(!CoralModel::getModel().getPowerState());
                    break;
                case MENU_ID_SETTINGS:
                    CoralController::getInstance().showSettingsDialog();
                    break;
                case MENU_ID_DONATE:
                    CoralController::getInstance().showDonateDialog();
                    break;
                case MENU_ID_EXIT:
                    CoralController::getInstance().exit();
                    break;
            }
        }
    }
...
namespace
{
    ...
    {
        dispatchToJuce([] {
            CoralController::getInstance().setPowerState(!CoralModel::getModel().getPowerState());
        });
    }
...
#pragma GCC diagnostic pop
    ...
    GtkMenu* createTrayMenu()
    {
        ...
        g_menu_power = gtk_menu_item_new_with_label(
            CoralModel::getModel().getPowerState() ? "Power off" : "Power on");
...
void CoralSystemTrayView::timerCallback()
{
