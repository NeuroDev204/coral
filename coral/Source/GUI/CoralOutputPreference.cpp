...
CoralOutputDeviceRow::CoralOutputDeviceRow(CoralOutputPreferenceListModel& model) : up_button_("up", DrawableButton::ImageFitted), down_button_("down", DrawableButton::ImageFitted), output_preference_list_model_(model)
{
    ...
    preset_list_.onChange = [this]() {
        ...
        if (index >= 0)
        {
            ...
            if (device_config_.device_name == CoralController::getInstance().getOutputName())
            {
                CoralController::getInstance().setPreset(preset);
            }
        }
    };
...
void CoralOutputDeviceRow::update(int index, bool is_row_selected, const DeviceConfig& device_config)
{
    if (index < 0)
        return;
    ...
    device_name_.setEnabled(CoralController::getInstance().isOutputDeviceConnected(device_config.device_name));
    ...
    if (preset_list_.getNumItems() == 0)
    {
        ...
        for (auto i = 0; i < model.getPresetCount(); i++)
            preset_list_.addItem(CoralModel::getModel().getPreset(i).name, i + 1);
    }
    ...
    for (auto i = 0; i < model.getPresetCount(); i++)
    {
        if (CoralModel::getModel().getPreset(i).name == device_config.preset)
        {
            selected_id = i + 1;
            break;
        }
...
void CoralOutputDeviceRow::paint(Graphics& g)
{
    if (is_row_selected_)
    {
...
CoralOutputPreferenceListModel::CoralOutputPreferenceListModel()
{
    DeviceConfig::onDeviceConfigsUpdate = [this]() {
        device_configs_ = CoralController::getInstance().getDeviceConfigs();
        if (onModelChanged)
            onModelChanged();
    };
    ...
    CoralController::getInstance().removeDeviceConfigs();
...
int CoralOutputPreferenceListModel::getNumRows()
{
    return device_configs_.size();
}

void CoralOutputPreferenceListModel::paintListBoxItem(int rowNumber, juce::Graphics& g, int width, int height, bool rowIsSelected)
{
}

Component* CoralOutputPreferenceListModel::refreshComponentForRow(int rowNumber, bool isRowSelected, Component* existingComponent)
{
    auto* row = dynamic_cast<CoralOutputDeviceRow*>(existingComponent);

    if (row == nullptr)
        row = new CoralOutputDeviceRow(*this);
...
void CoralOutputPreferenceListModel::moveRowUp(int index)
{
    if (index <= 0)
        return;
...
void CoralOutputPreferenceListModel::moveRowDown(int index)
{
...
void CoralOutputPreferenceListModel::modelChanged(CoralModel::Event event)
{
    if (event == CoralModel::Event::OutputListUpdated || event == CoralModel::Event::PresetListUpdated)
    {
        if (onModelChanged)
            onModelChanged();
    }
...
void CoralOutputPreferenceListModel::updateDeviceConfig(const DeviceConfig device_config)
{
...
void CoralOutputPreferenceListModel::persist()
{
    CoralController::getInstance().saveDeviceConfigs(device_configs_);
}

CoralOutputPreference::CoralOutputPreference()
{
...
void CoralOutputPreference::update()
{
    output_preference_list_.updateContent();
...
bool CoralOutputPreference::keyPressed(const KeyPress& key, Component*)
{
...
void CoralOutputPreference::resized()
{
    output_preference_list_.setBounds(getLocalBounds().reduced(5, 10));
    output_preference_list_.updateContent();
}

void CoralOutputPreference::paint(Graphics& g)
{
