...
const uint32 CoralTheme::theme_colors_[CoralThemeMode::NumModes][CoralColor::NumColors] =
...
const char* CoralTheme::theme_images_[CoralThemeMode::NumModes][CoralImage::NumImages] =
...
const int CoralTheme::theme_image_sizes_[CoralThemeMode::NumModes][CoralImage::NumImages] =
...
CoralThemeMode CoralTheme::theme_mode_ = CoralThemeMode::Dark;

//==============================================================================
CoralTheme::CoralTheme() : LookAndFeel_V4()
{
	init();
}

void CoralTheme::init()
{
...
LookAndFeel_V4::ColourScheme CoralTheme::getFxColourScheme()
{
...
Label* CoralTheme::createComboBoxTextBox(ComboBox& box)
{
...
Font CoralTheme::getComboBoxFont(ComboBox& box)
{
	ignoreUnused(box);
	return Font(font_600_).withHeight(14.0f);
}

void CoralTheme::positionComboBoxText(ComboBox& box, Label& label)
{
...
void CoralTheme::drawComboBox(Graphics& g, int width, int height, bool,
...
void CoralTheme::drawComboBoxTextWhenNothingSelected(Graphics& g, ComboBox& box, Label& label)
{
...
void CoralTheme::drawLinearSlider(Graphics& g, int x, int y, int width, int height,
...
void CoralTheme::drawRotarySlider(Graphics& g, int x, int y, int width, int height, float sliderPos,
...
int CoralTheme::getSliderThumbRadius(Slider& slider)
{
...
Slider::SliderLayout CoralTheme::getSliderLayout(Slider& slider)
{
...
void CoralTheme::drawPopupMenuItem(Graphics& g, const juce::Rectangle<int>& area, bool is_separator, bool is_active,
...
Font CoralTheme::getPopupMenuFont()
{
	return Font(font_600_).withHeight(17.0f);
}

void CoralTheme::preparePopupMenuWindow(Component& new_window)
{
...
void CoralTheme::loadFont(String language)
{
...
Font CoralTheme::getTextButtonFont(TextButton&, int button_height)
{
	return Font(font_400_.get()).withHeight(jmin(15.0f, (float)button_height * 0.55f + 4.0f));
}

void CoralTheme::drawButtonBackground(Graphics& g, Button& button, const Colour& backgroundColour,
...
Font CoralTheme::getNormalFont()
{
	return Font(font_400_).withHeight(14.5f);
}

Font CoralTheme::getSmallFont()
{
	return Font(font_400_).withHeight(12.5f);
}

Font CoralTheme::getTitleFont()
{
	return Font(font_400_).withHeight(15.0f);
}

void CoralTheme::paintCoralMark(Graphics& g, Rectangle<float> bounds, Colour colour)
{
	g.setColour(colour);
...
void CoralTheme::paintGlassCard(Graphics& g, Rectangle<float> bounds, float radius)
{
...
Typeface::Ptr CoralTheme::getDefaultTypeface()
{
    return font_400_;
}

CoralThemeMode CoralTheme::getThemeMode()
{
	return theme_mode_;
}

void CoralTheme::setThemeMode(CoralThemeMode theme_mode)
{
...
uint32 CoralTheme::getColor(CoralColor color)
{
	return theme_colors_[theme_mode_][color];
}

const char* CoralTheme::getImage(CoralImage image)
{
	return theme_images_[theme_mode_][image];
}

const int CoralTheme::getImageSize(CoralImage image)
{
	return theme_image_sizes_[theme_mode_][image];
}

Rectangle<int> CoralTheme::getTooltipBounds(const String& tipText, Point<int> screenPos, Rectangle<int> parentArea)
{
...
void CoralTheme::drawTooltip(Graphics& g, const String& text, int width, int height)
{
...
void CoralTheme::drawDocumentWindowTitleBar(DocumentWindow& window, Graphics& g,
...
class CoralTheme_DocumentWindowButton : public Button
{
public:
	CoralTheme_DocumentWindowButton(const String& name, Colour c, const Path& normal, const Path& toggled)
...
private:
	...
	JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(CoralTheme_DocumentWindowButton)
};

Button* CoralTheme::createDocumentWindowButton(int buttonType)
{
	Path shape;
	...
	if (buttonType == DocumentWindow::closeButton)
	{
		...
		return new CoralTheme_DocumentWindowButton("close", Colour(FXCOLOR(DefaultFill)).withAlpha(1.0f), shape, shape);
	}
	...
	if (buttonType == DocumentWindow::minimiseButton)
	{
		...
		return new CoralTheme_DocumentWindowButton("minimise", Colour(FXCOLOR(DefaultFill)).withAlpha(1.0f), shape, shape);
	}
	...
	if (buttonType == DocumentWindow::maximiseButton)
	{
		...
		return new CoralTheme_DocumentWindowButton("maximise", Colour(FXCOLOR(DefaultFill)).withAlpha(1.0f), shape, fullscreenShape);
	}
...
TextLayout CoralTheme::layoutTooltipText(const String& text, Colour colour) noexcept
{
...
Typeface::Ptr CoralTheme::loadTypeface(String fileName)
{
