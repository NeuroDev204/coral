...
CoralPresetExportDialog::CoralPresetExportDialog() : CoralWindow("Export Presets")
{
...
bool CoralPresetExportDialog::keyPressed(const KeyPress& key)
{
...
void CoralPresetExportDialog::closeButtonPressed()
{
    exitModalState(0);
    removeFromDesktop();
}

CoralPresetExportDialog::PresetExportProgress::PresetExportProgress()
{
    setFramesPerSecond(30);
    colour_gradient_start_ = 0.0;
}

void CoralPresetExportDialog::PresetExportProgress::update()
{
    colour_gradient_start_ += 0.01f;
}

void CoralPresetExportDialog::PresetExportProgress::paint(Graphics& g)
{
...
CoralPresetExportDialog::PresetExportComponent::PresetExportComponent() : export_button_(TRANS("Export"))
{
...
void CoralPresetExportDialog::PresetExportComponent::resized()
{
...
int CoralPresetExportDialog::PresetExportComponent::getNumRows()
{
    return CoralModel::getModel().getPresetCount();
}

void CoralPresetExportDialog::PresetExportComponent::paintListBoxItem(int rowNumber, Graphics &g, int width, int height, bool rowIsSelected)
{
    if (rowIsSelected)
    {
    ...
    auto preset = CoralModel::getModel().getPreset(rowNumber);
...
void CoralPresetExportDialog::PresetExportComponent::selectedRowsChanged(int)
{
...
void CoralPresetExportDialog::PresetExportComponent::buttonClicked(Button* button)
{
    if (button == &export_button_)
    {
        ...
        if (selected_presets.size() > 0)
        {
            ...
            {
                presets.add(CoralModel::getModel().getPreset(selected_presets[i]));
            }

            show_explorer = CoralController::getInstance().exportPresets(presets);
        }
        
        if (show_explorer)
        {
            CoralConfirmationMessage::showMessage(TRANS("Presets are exported successfully!"), CoralConfirmationMessage::Style::OK);
            File(File::addTrailingSeparator(File::getSpecialLocation(File::SpecialLocationType::userDocumentsDirectory).getFullPathName()) + L"Coral\\Presets\\Export\\").revealToUser();
        }
