...
CoralEqualizer::CoralEqualizer()
{
    ...
    for (int i = 0; i < num_bands; i++)
    {
        ...
        band_boosts_[i].reset(new CoralEqSlider(i, MAX_GAIN));
        center_frequencies_[i].reset(new CoralBandCenterFreqSlider(i, *labels_[i]));
        ...
        CoralController::getInstance().getEqBandFrequencyRange(i, &min_freq, &max_freq);
...
void CoralEqualizer::reinit(int num_bands)
{   
    auto& controller = CoralController::getInstance();
...
void CoralEqualizer::sliderValueChanged(Slider* slider)
{

}

void CoralEqualizer::sliderDragStarted(Slider* slider)
{
    if (highlight_mode_)
    {
        return;
...
void CoralEqualizer::sliderDragEnded(Slider* slider)
{
    if (highlight_mode_)
    {
        auto& controller = CoralController::getInstance();
...
void CoralEqualizer::timerCallback()
{
    auto& controller = CoralController::getInstance();
...
void CoralEqualizer::showValues(bool show)
{
...
void CoralEqualizer::resized()
{
    ...
    for (auto i=0; i<labels_.size(); i++)
    {
        ...
        if (show_center_frequencies)
        {
            ...
            const int slider_h = jmax(80, bounds.getHeight() - Y_MARGIN - LABEL_HEIGHT - rotary_width - 16);
            band_boosts_[i]->setBounds(x + (width - CoralTheme::SLIDER_THUMB_RADIUS * 4) / 2, Y_MARGIN, CoralTheme::SLIDER_THUMB_RADIUS * 4, slider_h);
        ...
        else
        {
            ...
            const int slider_h = jmax(80, bounds.getHeight() - Y_MARGIN - LABEL_HEIGHT * 2 - 12);
            band_boosts_[i]->setBounds(x + (width - CoralTheme::SLIDER_THUMB_RADIUS * 4) / 2, Y_MARGIN, CoralTheme::SLIDER_THUMB_RADIUS * 4, slider_h);
...
void CoralEqualizer::paint(Graphics& g)
{
    ...
    for (auto i = 0; i < band_boosts_.size(); i++) 
    {
        ...
        if (i == 0)                                           
        {
            path.startNewSubPath(Point<float>(x, band_boosts_[i]->getBottom() - CoralTheme::SLIDER_THUMB_RADIUS));
        }
        ...
        {
            path.lineTo(x, band_boosts_[i]->getBottom() - CoralTheme::SLIDER_THUMB_RADIUS);
        }
    }
    path.closeSubPath();
...
CoralEqualizer::CoralEqSlider::CoralEqSlider(int band, float max_gain)
{
    band_ = band;
...
void CoralEqualizer::CoralEqSlider::setGainValue(float value)
{
    ...
    auto y = getPositionOfValue(value) - (CoralTheme::SLIDER_THUMB_RADIUS *3);
    gain_label_.setBounds(gain_label_.getBounds().withY(y));
}

void CoralEqualizer::CoralEqSlider::showValue(bool show)
{
    gain_label_.setVisible(show && isEnabled());
}

void CoralEqualizer::CoralEqSlider::enablementChanged()
{
    if (isEnabled())
    {
        setMouseCursor(MouseCursor::PointingHandCursor);
...
void CoralEqualizer::CoralEqSlider::resized()
{
    Slider::resized();
    gain_label_.setBounds(gain_label_.getX(), gain_label_.getY(), getWidth(), LABEL_HEIGHT);
}

void CoralEqualizer::CoralEqSlider::valueChanged()
{
    auto value = getValue();
    if (value != CoralController::getInstance().getEqBandBoostCut(band_))
    {
        CoralController::getInstance().setEqBandBoostCut(band_, value);
...
bool CoralEqualizer::CoralEqSlider::keyPressed(const KeyPress& key)
{
    if (isEnabled())
    {
...
void CoralEqualizer::CoralEqSlider::mouseDown(const juce::MouseEvent& event)
{
...
CoralEqualizer::CoralBandCenterFreqSlider::CoralBandCenterFreqSlider(int band, Label& freq_label) : Slider(Slider::SliderStyle::Rotary, Slider::TextEntryBoxPosition::NoTextBox), freq_label_(freq_label)
{
    band_ = band;
...
void CoralEqualizer::CoralBandCenterFreqSlider::setFrequency(float value)
{
    setValue(value, NotificationType::dontSendNotification);
    ...
    if (value > 0)
    {
        int nBands = CoralController::getInstance().getNumEqBands();
...
void CoralEqualizer::CoralBandCenterFreqSlider::enablementChanged()
{
    if (isEnabled())
    {
        setMouseCursor(MouseCursor::PointingHandCursor);
...
void CoralEqualizer::CoralBandCenterFreqSlider::valueChanged()
{
    auto freq = getValue();

    if (freq != CoralController::getInstance().getEqBandFrequency(band_))
    {
        CoralController::getInstance().setEqBandFrequency(band_, freq);

        setFrequency(freq);
    }
}

bool CoralEqualizer::CoralBandCenterFreqSlider::keyPressed(const KeyPress& key)
{
    if (isEnabled())
    {
        if (key.isKeyCode(KeyPress::upKey))
...
void CoralEqualizer::CoralBandCenterFreqSlider::mouseDown(const juce::MouseEvent& event)
{
