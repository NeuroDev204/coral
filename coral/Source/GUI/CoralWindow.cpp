...
//==============================================================================
CoralWindow::CoralWindow(String name, bool desktop_controls) : title_bar_(name, desktop_controls), content_(nullptr)
{
	...
	title_bar_.setSize(64, CoralTheme::TITLE_BAR_HEIGHT);
...
CoralWindow::~CoralWindow()
{
	setLookAndFeel(nullptr);
}

void CoralWindow::setContent(Component* content)
{
	if (content != content_)
	{
...
void CoralWindow::setHeaderCenter(Component* component)
{
	title_bar_.setCenterComponent(component);
}

void CoralWindow::minimiseButtonPressed()
{
	closeButtonPressed();
}

void CoralWindow::maximiseButtonPressed()
{
	if (maximised_)
	{
...
void CoralWindow::startLogoAnimation()
{
	title_bar_.startLogoAnimation();
}

void CoralWindow::stopLogoAnimation()
{
	title_bar_.stopLogoAnimation();
}

void CoralWindow::addToolbarButton(Button* toolbarButton, bool right_aligned)
{
	title_bar_.addToolbarButton(toolbarButton, right_aligned);
}

void CoralWindow::enableShadow(bool enable)
{
    draw_shadow_ = enable;
    if (enable)
    {
...
bool CoralWindow::isShadowEnabled()
{
    return draw_shadow_;
}

void CoralWindow::paint(Graphics& g)
{
	auto bounds = getLocalBounds().toFloat();
	const float radius = (float)CoralTheme::WINDOW_CORNER_RADIUS;

	g.setColour(Colour(FXCOLOR(WindowBackground)).withAlpha(CoralTheme::WINDOW_FILL_ALPHA));
...
void CoralWindow::resized()
{
	title_bar_.setBounds(0, 0, getWidth(), CoralTheme::TITLE_BAR_HEIGHT);
	if (content_ != nullptr)
...
void CoralWindow::WindowButton::paintButton(Graphics& g, bool over, bool down)
{
...
void CoralWindow::GlassGlyphButton::paintButton(Graphics& g, bool over, bool down)
{
...
CoralWindow::TitleBar::TitleBar(String name, bool desktop_controls)
{
	name_ = name;
...
void CoralWindow::TitleBar::startLogoAnimation()
{
	if (animation_icon_.get() != nullptr)
	{
...
void CoralWindow::TitleBar::stopLogoAnimation()
{
	if (animation_icon_.get() != nullptr)
	{
...
void CoralWindow::TitleBar::addToolbarButton(Button* toolbarButton, bool right_aligned)
{
...
void CoralWindow::TitleBar::setCenterComponent(Component* component)
{
	center_ = component;
	if (center_ != nullptr)
...
void CoralWindow::TitleBar::paint(Graphics& g)
{
	...
	CoralTheme::paintCoralMark(g, { 12.0f, (float) (getHeight() - 16) * 0.5f, 20.0f, 16.0f },
...
void CoralWindow::TitleBar::resized()
{
...
void CoralWindow::TitleBar::lookAndFeelChanged()
{
	updateLogo();
	repaint();
}

void CoralWindow::TitleBar::buttonClicked(Button* button)
{
	auto parent = dynamic_cast<CoralWindow*>(getParentComponent());
	if (parent == nullptr)
		return;
...
void CoralWindow::TitleBar::mouseDown(const MouseEvent& e)
{
	dragging_ = true;
	auto parent = dynamic_cast<CoralWindow*>(getParentComponent());
...
void CoralWindow::TitleBar::mouseDrag(const MouseEvent& e)
{
	if (dragging_)
	{
		auto parent = dynamic_cast<CoralWindow*>(getParentComponent());
...
void CoralWindow::TitleBar::mouseUp(const MouseEvent&)
{
	dragging_ = false;
}

void CoralWindow::TitleBar::updateLogo()
{
	icon_.reset();
