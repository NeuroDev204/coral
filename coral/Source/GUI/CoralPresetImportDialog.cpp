...
class CoralImportCompleteMessage : public CoralWindow
{
public:
    CoralImportCompleteMessage(const StringArray& imported_presets, const StringArray& skipped_presets)
    ...
    ~CoralImportCompleteMessage() = default;
...
private:
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(CoralImportCompleteMessage)
};

CoralPresetImportDialog::CoralPresetImportDialog() : CoralWindow("Import Presets")
{
...
bool CoralPresetImportDialog::keyPressed(const KeyPress& key)
{
...
void CoralPresetImportDialog::closeButtonPressed()
{
    exitModalState(0);
    removeFromDesktop();
}

CoralPresetImportDialog::PresetImportComponent::PresetImportComponent() : 
...
void CoralPresetImportDialog::PresetImportComponent::resized()
{
    auto bounds = getLocalBounds();
    bounds.setTop(10);
    bounds.reduce(20, 0);
...
void CoralPresetImportDialog::PresetImportComponent::buttonClicked(Button* button)
{
    if (button == &import_button_)
    {
        ...
        {
            CoralConfirmationMessage::showMessage(TRANS("Preset files not found in the selected folder."), CoralConfirmationMessage::Style::OK);
            return;
        }
        StringArray imported_presets;
        StringArray skipped_presets;

        CoralController::getInstance().importPresets(preset_paths, imported_presets, skipped_presets);

        Component::getParentComponent()->exitModalState(0);
        Component::getParentComponent()->removeFromDesktop();

        CoralImportCompleteMessage import_complete_message(imported_presets, skipped_presets);
