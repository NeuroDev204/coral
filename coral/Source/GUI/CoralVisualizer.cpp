...
CoralVisualizer::CoralVisualizer()
{
    band_values_.resize(CoralController::NUM_SPECTRUM_BANDS);
    band_graph_.resize(CoralController::NUM_SPECTRUM_BANDS * NUM_BARS);
...
void CoralVisualizer::reset()
{
    for (int i = 0; i < CoralController::NUM_SPECTRUM_BANDS * NUM_BARS; i++)
    {
        band_graph_.set(i, 0);
    }
}

void CoralVisualizer::update()
{
    if (!isEnabled())
        return;

    CoralController::getInstance().getSpectrumBandValues(band_values_);

    for (int i = 0; i < CoralController::NUM_SPECTRUM_BANDS; i++)
    {
...
void CoralVisualizer::paint(Graphics& g)
{
    auto bounds = getLocalBounds();

    g.setFillType(FillType(Colours::white.withAlpha(0.06f)));
    ...
    for (auto i = 0; i < CoralController::NUM_SPECTRUM_BANDS * NUM_BARS; i++)
    {
...
void CoralVisualizer::enablementChanged()
{
    if (isEnabled())
    {
        start();
...
void CoralVisualizer::lookAndFeelChanged()
{
    calcGradient();
    repaint();
}

void CoralVisualizer::calcGradient()
{
    float alpha = 0.55f;
    if (CoralController::getInstance().isAudioProcessing())
        alpha = 1.0f;
