...
int DfxDspPrivate::eqSetProcessingOn(int i_storage_type, int i_on)
{
	...
	if ((i_storage_type == DFXP_STORAGE_TYPE_REGISTRY) ||
		(i_storage_type == DFXP_STORAGE_TYPE_ALL))
	{
		swprintf(wcp_full_key_path, sizeof(wcp_full_key_path)/sizeof(wchar_t), PT_WF L"\\" PT_WF L"\\%d\\%d\\" PT_WF L"\\" PT_WF L"\\" PT_WF,
		...
		swprintf(wcp_key_value, sizeof(wcp_key_value)/sizeof(wchar_t), L"%d", i_on);
