...
int PT_DECLSPEC valsSave(PT_HANDLE *hp_vals, wchar_t *wcp_dir_path, wchar_t *wcp_filename)
{
	...
	swprintf(fullpath_str, 1024, L"%s\\%s", wcp_dir_path, wcp_filename);
...
int PT_DECLSPEC valsCalcDateStrings(wchar_t *wcp_filename, wchar_t *wcp_formatted)
{
	 ...
	 */
	swprintf(wcp_short_sec_str, 1024, L"%ld", seconds);
	length = (int)wcslen(wcp_short_sec_str);
	if (length == 5)
		swprintf(wcp_sec_str, 1024, L"%c%c.%c%c%c",
			wcp_short_sec_str[0], wcp_short_sec_str[1],
			wcp_short_sec_str[2], wcp_short_sec_str[3], wcp_short_sec_str[4]);
	else if (length == 4)
		swprintf(wcp_sec_str, 1024, L"0%c.%c%c%c",
			wcp_short_sec_str[0], wcp_short_sec_str[1],
			wcp_short_sec_str[2], wcp_short_sec_str[3]);
	else if (length == 3)
		swprintf(wcp_sec_str, 1024, L"00.%c%c%c",
			wcp_short_sec_str[0], wcp_short_sec_str[1],
			wcp_short_sec_str[2]);
	else if (length == 2)
		swprintf(wcp_sec_str, 1024, L"00.0%c%c",
			wcp_short_sec_str[0], wcp_short_sec_str[1]);
	else if (length == 1)
		swprintf(wcp_sec_str, 1024, L"00.00%c",
			wcp_short_sec_str[0]);
	else if (length == 0)
		swprintf(wcp_sec_str, 1024, L"00.000");

	swprintf(wcp_filename, 1024, L"%s%s", wcp_date_str, wcp_sec_str);
...
int PT_DECLSPEC valsRead(wchar_t *wcp_file_path, int i_trace_mode, CSlout *hp_slout, PT_HANDLE **hpp_vals)
{
	...
	{
		swprintf(cast_handle->wcp_msg1, 1024, L"valsRead(): Entered, wcp_file_path = %s", wcp_file_path);
	...
	/* Get the Comment. */
	if (fgetws(wcp_str, LINE_LENGTH, stream) == NULL)
	{
		fclose(stream);
		return(NOT_OKAY);
	}
	...
	if (str_length > 0)
	{
		...
		swprintf(cast_handle->wcp_comment, 1024, L"%s", wcp_str);
	}
	else
	...
	if (cast_handle->file_version >= 9)
	{
		...
		if (cast_handle->hp_graphicEq != NULL)
		{
			if (cast_handle->i_trace_mode)
			{
				swprintf(cast_handle->wcp_msg1, 1024, L"valsRead(): Calling GraphicEqFreeUp()");
				(cast_handle->slout_hdl)->Message_Wide(FIRST_LINE, cast_handle->wcp_msg1);
			}
