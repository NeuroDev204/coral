					...
					CSlout *hp_slout)
{
    ...
    /* Store the filepath */
    swprintf(cast_handle->wcp_filepath, 1024, L"%s", wcp_file_path);
	...
	swprintf((cast_handle->wcp_title), 1024, L"%s", wcp_str);
	...
	if (str_length == 0)
	{
		swprintf(cast_handle->wcp_msg1, 1024, L"Illegal Mono/Stereo Input Specification");
	...
	if (str_length == 0)
	{
		swprintf(cast_handle->wcp_msg1, 1024, L"Illegal Mono/Stereo Output Specification");
		(cast_handle->slout_hdl)->Error_Wide(FIRST_LINE, cast_handle->wcp_msg1);
	}
	if ((wcp_str[0] == L's') || (wcp_str[0] == L'S'))
...
int PT_DECLSPEC valsCfgCheckFileType(wchar_t *wcp_file_path, int *ip_proper_type)
{
	...
	*ip_proper_type = IS_FALSE;
	...
	if (_wfopen_s(&stream, wcp_file_path, L"r") != 0)
		return(OKAY);
#else
	stream = fileOpen_Wide(wcp_file_path, (wchar_t *)L"r", NULL);
	if (stream == NULL)
		return(OKAY);
#endif
...
int PT_DECLSPEC valsCfgGetTitle(PT_HANDLE *hp_vals_cfg, wchar_t *wcp_title)
{
	...
	swprintf(wcp_title, 1024, L"%s", cast_handle->wcp_title);
