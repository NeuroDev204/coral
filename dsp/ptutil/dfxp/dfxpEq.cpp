...
int dfxpEqSetProcessingOn(PT_HANDLE *hp_dfxp, int i_storage_type, int i_on)
{
	...
	if ((i_storage_type == DFXP_STORAGE_TYPE_REGISTRY) ||
		 (i_storage_type == DFXP_STORAGE_TYPE_ALL))
	{
		swprintf(wcp_full_key_path, 256, PT_WF L"\\" PT_WF L"\\%d\\%d\\" PT_WF L"\\" PT_WF L"\\" PT_WF, 
		...
		swprintf(wcp_key_value, 256, L"%d", i_on);
...
{	
		 ...
		 (i_storage_type == DFXP_STORAGE_TYPE_ALL))
	{
		...
		swprintf(wcp_keyname, 256, PT_WF L"%d", DFXP_REGISTRY_EQ_BAND_NAME_WIDE, i_band_num);
		...
		swprintf(wcp_key_value, 256, L"%.2f", r_boost_cut);
...
{	
	...
	*rp_boost_cut = (realtype)0.0;
	swprintf(wcp_boost_cut, 256, L"%.2f", *rp_boost_cut);
...
{	
	...
	*rp_boost_cut = (realtype)0.0;
	...
	swprintf(wcp_boost_cut, 256, PT_WF, wcp_key_value);
