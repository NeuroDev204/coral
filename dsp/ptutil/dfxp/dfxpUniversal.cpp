...
*/
	 ...
	 */
	b_lean_and_mean = IS_FALSE;
	if (!cast_handle->processing_only)
	{
		b_lean_and_mean = IS_TRUE;
	}
	...
	if (!b_lean_and_mean)
	{
		if (i_check_for_duplicate_buffers)
		{
			...
			if (cast_handle->trace.mode)
			{
				swprintf(cast_handle->wcp_msg1, 256, L"dfxpUniversalModifySamples: processed hash value = %d", cast_handle->universal.hash_queue_vals[cast_handle->universal.hash_queue_index]);
				(cast_handle->slout1)->Message_Wide(FIRST_LINE, cast_handle->wcp_msg1);	
			}
...
{	
	...
	if (cast_handle->hp_sharedUtil != NULL)
	{
		...
		if (dfxSharedUtilGetTotalProcessedTime(cast_handle->hp_sharedUtil, &ul_previous_total_processed_time_msecs) == OKAY)
		{
			...
			dfxSharedUtilSetTotalProcessedTime(cast_handle->hp_sharedUtil, ul_new_total_processed_time_msecs);
		}
	}
...
{	
	...
	*ip_allow_processing = IS_TRUE;
	...
	if (cast_handle->trace.mode)
	{
		swprintf(cast_handle->wcp_msg1, 256, L"dfxpUniversalCheckParentCompatibility: allow_processing = %d", *ip_allow_processing);
		(cast_handle->slout1)->Message_Wide(FIRST_LINE, cast_handle->wcp_msg1);
	}
