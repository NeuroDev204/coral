								   	         ...
								   	         int i_num_sample_sets)
{
   ...
   if (!(cast_handle->fully_initialized))
	...
	if ((cast_handle->trace.mode) && (!cast_handle->trace.i_process_int_samples_done))
	{
		swprintf(cast_handle->wcp_msg1, 256, L"dfxpModifyShortIntSamples(): i_num_sample_sets = %d", i_num_sample_sets);
	...
	if ((cast_handle->trace.mode) && (!cast_handle->trace.i_process_int_samples_done))
	{
		swprintf(cast_handle->wcp_msg1, 256, L"dfxpModifyShortIntSamples(): Total Num Samples = %d", total_num_samples);
	   (cast_handle->slout1)->Message_Wide(FIRST_LINE, cast_handle->wcp_msg1);
   }
