				 ...
				 wchar_t *wcp_user_dir, int i_user_min_index, int i_user_max_index)
{
	...
	if (cast_handle->wcp_factory_dir == NULL)
		return(NOT_OKAY);
	swprintf(cast_handle->wcp_factory_dir, 256, L"%s", wcp_factory_dir);
	...
	if (cast_handle->wcp_user_dir == NULL)
		return(NOT_OKAY);
	swprintf(cast_handle->wcp_user_dir, 256, L"%s", wcp_user_dir);
...
int PT_DECLSPEC prelstConstructFullpath(PT_HANDLE *hp_prelst, int i_index, wchar_t *wcp_fullpath)
{
   ...
   if (prelstAskIsFactory(hp_prelst, i_index, &is_factory) != OKAY)
	   return(NOT_OKAY);
   if (is_factory)
      swprintf(wcp_fullpath, 256, L"%s\\%s", cast_handle->wcp_factory_dir, wcp_filename);
   else
      swprintf(wcp_fullpath, 256, L"%s\\%s", cast_handle->wcp_user_dir, wcp_filename);
...
int PT_DECLSPEC prelstConstructFilename(PT_HANDLE *hp_prelst, int i_index, wchar_t *wcp_filename)
{
   ...
   if (i_index < cast_handle->user_min_index)
	  swprintf(wcp_filename, 256, L"%d.%s", i_index + 1, PRELST_FACTORY_EXTENSION_WIDE);
   else
      swprintf(wcp_filename, 256, L"%d.%s", i_index + 1, PRELST_USER_EXTENSION_WIDE);
