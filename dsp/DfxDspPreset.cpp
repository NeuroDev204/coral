...
int DfxDspPrivate::getStateInfoFromVals(PT_HANDLE *hp_vals, bool b_include_eq)
{
	...
	*/
	...
	/* Synchronize all loaded effect values into the virtual registry so
	 * dfxpCommunicateAll() picks them up correctly when triggered by
	 * processTimer(). Without this, the virtual registry (simulated on Linux)
	 * would retain default values (0) and bypass flags would be wrong.
	 */
	dfxpSetButtonValue(dfxp_handle_, DFX_UI_BUTTON_FIDELITY,     !(fidelity_.bypass));
	dfxpSetKnobValue(dfxp_handle_, DFX_UI_KNOB_FIDELITY,         fidelity_.value, false);

	dfxpSetButtonValue(dfxp_handle_, DFX_UI_BUTTON_AMBIENCE,     !(ambience_.bypass));
	dfxpSetKnobValue(dfxp_handle_, DFX_UI_KNOB_AMBIENCE,         ambience_.value, false);

	dfxpSetButtonValue(dfxp_handle_, DFX_UI_BUTTON_SURROUND,     !(surround_.bypass));
	dfxpSetKnobValue(dfxp_handle_, DFX_UI_KNOB_SURROUND,         surround_.value, false);

	dfxpSetButtonValue(dfxp_handle_, DFX_UI_BUTTON_DYNAMIC_BOOST, !(dynamic_boost_.bypass));
	dfxpSetKnobValue(dfxp_handle_, DFX_UI_KNOB_DYNAMIC_BOOST,    dynamic_boost_.value, false);

	dfxpSetButtonValue(dfxp_handle_, DFX_UI_BUTTON_BASS_BOOST,   !(bass_boost_.bypass));
	dfxpSetKnobValue(dfxp_handle_, DFX_UI_KNOB_BASS_BOOST,       bass_boost_.value, false);

	dfxpSetButtonValue(dfxp_handle_, DFX_UI_BUTTON_HEADPHONE,    headphone_on_);
...
DfxPreset DfxDspPrivate::getPresetInfo(std::wstring preset_file_full_path)
{
	...
	{
		// Failed to read - return empty preset
		return dfx_preset;
	}

	if (vals_hdl == NULL)
	{
		// Handle is NULL - return empty preset
		return dfx_preset;
	}
	...
	{
		// Failed to get comment - cleanup and return
		valsFreeUp(&vals_hdl);
		return dfx_preset;
	}

	// Only assign if wcp_comment is NOT NULL
	// Assigning NULL to std::wstring causes crash!
	if (wcp_comment != NULL && wcslen(wcp_comment) > 0)
	{
