...
FILE PT_DECLSPEC *fileOpen_Wide(wchar_t *wcp_name, wchar_t *wcp_mode, CSlout *hp_slout)
{
	...
	char *cp_name = nullptr;
	char *cp_mode = nullptr;

	// Convert wide strings to narrow strings
	size_t name_len = wcstombs(nullptr, wcp_name, 0) + 1;
	size_t mode_len = wcstombs(nullptr, wcp_mode, 0) + 1;

	if (name_len > 0 && mode_len > 0) {
		cp_name = (char*)malloc(name_len);
		cp_mode = (char*)malloc(mode_len);

		if (cp_name && cp_mode) {
			wcstombs(cp_name, wcp_name, name_len);
			wcstombs(cp_mode, wcp_mode, mode_len);

			stream = fopen(cp_name, cp_mode);

			if (stream == nullptr && hp_slout != nullptr)
			{
				swprintf(wcp_file_msg, L"%s: %s", wcp_name, strerror(errno));
				hp_slout->Error_Wide(FIRST_LINE, wcp_file_msg);
			}
		} else {
			stream = nullptr;
...
int PT_DECLSPEC fileWaitOnWriteBlock_Wide(wchar_t *wcp_name, int *ip_timed_out, CSlout *hp_slout)
{
	...
	*ip_timed_out = IS_FALSE;
	...
	do
	{
		...
		fopen_result = _wfopen_s(&stream, wcp_name, L"w");
		current_time = time(NULL);
	}
	while ((fopen_result != 0) && (current_time < end_time));
#else
	// Linux implementation
	char *cp_name = nullptr;
	size_t name_len = wcstombs(nullptr, wcp_name, 0) + 1;
	if (name_len > 0) {
		...
		if (cp_name) {
			...
			do
			{
				usleep(FILE_WAIT_ON_WRITE_BLOCK_SLEEP_INTERVAL_MSEC * 1000);
				stream = fopen(cp_name, "w");
				fopen_result = (stream == nullptr) ? -1 : 0;
				current_time = time(NULL);
			}
			while ((fopen_result != 0) && (current_time < end_time));
