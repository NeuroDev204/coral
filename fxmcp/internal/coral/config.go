...
// BuildCommandLine converts a set of CoralController.cpp applyConfig flag
...
// preset/device names; CoralController's ArgumentList parsing strips a
// matching pair of quotes via .unquoted() before use.
//
// A value containing a literal `"` character is rejected outright rather
// than quoted, escaped, or stripped. CoralController's own re-tokenizer (see
...
// Coral.exe -- an argument-injection path, since these values can
...
// matter to CoralController's flag-by-name parsing.
...
// Apply builds a command line from flags and sends it to Coral.exe.
//
...
// invocation works whether or not Coral is already running.
func Apply(ctx context.Context, paths *Paths, flags map[string]string, requireRunning bool) error {
	...
	if _, err := Run(ctx, applyConfigTimeout, paths.CoralExe, args...); err != nil {
		return err
	}
	return nil
}

// EnsureRunning makes sure a live Coral instance is up and responding
// before a caller sends a command that only applyConfig (not initConfig)
// understands. If Coral isn't running, it launches one with no
...
// read, not just process existence, since CoralController's model needs to
...
func EnsureRunning(ctx context.Context, paths *Paths) error {
	...
	if err := launch(paths.CoralExe); err != nil {
		return newError(ErrKindRequiresRunningInstance, err, "launch Coral.exe")
	}

	if err := pollUntil(ctx, launchPollTotal, launchPollTick, func() (bool, error) {
		return IsCoralRunning()
	}); err != nil {
		return newError(ErrKindRequiresRunningInstance, err, "Coral.exe did not start within %s", launchPollTotal)
	}
	...
	if err := pollUntil(ctx, readyPollTotal, readyPollTick, func() (bool, error) {
		...
		return newError(ErrKindRequiresRunningInstance, lastErr, "Coral.exe did not become ready within %s", readyPollTotal)
	}
	return nil
}
...
// naming label if not. CoralController.cpp's applyConfig silently resets an
...
// ValidateNumEqBands checks n is one of the band counts CoralController
