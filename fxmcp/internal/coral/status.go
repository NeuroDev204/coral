...
// CoralController stores internally.
type Effects struct {
...
// Status is the shape of %APPDATA%\Coral\status.json, written by
// CoralController::printStatus (the --status command-line option).
type Status struct {
...
func statusFilePath() (string, error) {
	...
	return filepath.Join(appData, "Coral", "status.json"), nil
}

// ReadStatus spawns "Coral.exe --status" and reads back the resulting
// %APPDATA%\Coral\status.json.
//
// --status requires an already-running instance (see IsCoralRunning); a
...
func ReadStatus(ctx context.Context, paths *Paths) (*Status, error) {
	...
	if _, err := Run(ctx, statusTimeout, paths.CoralExe, "--status"); err != nil {
		return nil, err
	}
	...
	return nil, newError(ErrKindRequiresRunningInstance, nil, "status.json not updated after --status within %s (is Coral running?)", statusPollTotal)
}
