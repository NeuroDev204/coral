...
// effectAliases maps every name CoralController.cpp's applyConfig accepts
// for --set_effect (case-insensitively) to a single canonical name -- the
// same key --status reports it under (see CoralController.cpp:604-616 and
// CoralController.cpp:691-695, where the enum CoralEffects::Fidelity is
...
// NormalizeEffectName maps any of CoralController's accepted aliases for an
...
// range. CoralController.cpp silently leaves an out-of-range effect value
