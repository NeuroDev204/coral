...
// argument-injection path found in review: CoralController's tokenizer has
...
// tokens into the command line actually sent to Coral.exe. This must be
...
// -- the property coral_apply_settings (Iteration 7) relies on to send
...
func TestApplySpawnsExactlyOnce(t *testing.T) {
	...
	paths := &Paths{CoralExe: `C:\fake\Coral.exe`}
	flags := map[string]string{
		"preset": "Bass Booster",
		"output": "Speakers",
		"view":   "1",
