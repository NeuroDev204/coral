...
// spawnMu serializes every invocation of Coral.exe/fxdiag.exe this
// package makes. Several code paths -- a resource read of coral://status,
// a write tool's own EnsureRunning/ReadStatus precondition check, and
// EnsureRunning's readiness poll -- can all want to spawn "Coral.exe
...
//
// This matters because CoralController.cpp's applyConfig (and initConfig, on
...
// CharacterFunctions::findEndOfToken, and CoralController re-tokenizes that
...
// `"Speakers (2- Realtek(R) Audio)"` arrives at Coral with a stray
...
// "C:\Program Files\Coral\Coral\Coral.exe" contains a space and
...
// always returns promptly, and Coral.exe invocations either become the
...
func Run(ctx context.Context, timeout time.Duration, exePath string, args ...string) (stdout string, err error) {
	if exePath == "" {
		return "", newError(ErrKindAppNotFound, nil, "executable path not resolved (see coral.Locate)")
	}
...
// launch starts exePath (Coral.exe with no arguments) without waiting
// for it to exit.
//
// Deliberately takes no context: a cold launch becomes the persistent,
// long-running Coral instance, and must not be killed just because the
