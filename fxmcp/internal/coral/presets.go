...
// CoralController.cpp's applyConfig: strip the filesystem-reserved
...
// status. CoralController silently no-ops rather than erroring in both
...
// CoralController.cpp's applyConfig checks before acting on each preset
...
func ValidateSavePreset(status *Status) error {
	if !status.Power {
		return newError(ErrKindValueRejected, nil, "cannot save a preset while Coral's power is off")
	}
...
func ValidateOverwritePreset(status *Status) error {
	if !status.Power {
		return newError(ErrKindValueRejected, nil, "cannot overwrite a preset while Coral's power is off")
	}
...
func ValidateUndoPreset(status *Status) error {
	if !status.Power {
		return newError(ErrKindValueRejected, nil, "cannot undo preset changes while Coral's power is off")
	}
	info, _, found := SelectedPresetInfo(status)
	if !found {
		return newError(ErrKindValueRejected, nil, "could not determine the currently selected preset's state")
...
func ValidateRenamePreset(status *Status) error {
	if !status.Power {
		return newError(ErrKindValueRejected, nil, "cannot rename a preset while Coral's power is off")
	}
	info, isBuiltIn, found := SelectedPresetInfo(status)
	if !found {
		return newError(ErrKindValueRejected, nil, "could not determine the currently selected preset's state")
...
func ValidateDeletePreset(status *Status) error {
	if !status.Power {
		return newError(ErrKindValueRejected, nil, "cannot delete a preset while Coral's power is off")
	}
