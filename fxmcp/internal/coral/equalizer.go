...
// MinBandGainDb and MaxBandGainDb mirror CoralController.h's MIN_GAIN/MAX_GAIN
...
// consistent with CoralController.cpp's applyConfig, which drops an ENTIRE
// --set_band_freq/--set_band_gain batch if the pair count exceeds the
// current band count (CoralController.cpp: "pairs.size() <= getNumEqBands()"),
...
func ValidateBandBatch(numPairs, numBands int) error {
	if numPairs > numBands {
		return newError(ErrKindValueRejected, nil, "%d band setting(s) exceeds the current %d-band equalizer; Coral silently ignores the entire batch in this case, not just the excess entries", numPairs, numBands)
	}
	return nil
}

// ValidateBandIndex checks idx is a valid 0-based band index for the
// current equalizer configuration. CoralController's setEqBandFrequency and
...
// ValidateBandGain checks gainDb is within CoralController's fixed per-band
