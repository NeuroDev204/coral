...
For example, run `coral.exe --preset="Bass Booster"` at any time to switch the running instance's preset without restarting it or affecting any other setting.
...
| `--status` | *(flag, no value)* | Writes the running instance's current state (power, presets, output devices, equalizer, effects) as JSON to `%APPDATA%\Coral\status.json`; any other options passed alongside `--status` are ignored. |
...
```
coral.exe --preset="Bass Booster" --output="Speakers (Realtek High Definition Audio)" --run_minimized
```

Switch a running instance to Pro view and turn power on:

```
coral.exe --view=2 --power=1
```

Tune the equalizer on a running instance:

```
coral.exe --num_bands=15 --filter_q=2.0 --master_gain=3.0 --volume_leveling=2.0
```

Set individual equalizer band frequencies and gains on a running instance:

```
coral.exe --set_band_freq="0:60,1:150" --set_band_gain="0:3.0,1:-2.5"
```

Set effect levels on a running instance:

```
coral.exe --set_effect="bass:7.5,ambience:4.0"
```

Change the display language:

```
coral.exe --language=fr
```
