...
// didn't (a precondition CoralController.cpp would otherwise have silently
// no-op'd on).
...
func (a *App) registerWriteTools(s *mcp.Server) {
	mcp.AddTool(s, &mcp.Tool{
		Name: "coral_set_power",
		Description: "Turns Coral's audio processing (DFX) on or off. Works whether or not Coral is " +
			"currently running -- if it isn't, this launches it with power set accordingly.",
		Annotations: &mcp.ToolAnnotations{DestructiveHint: boolPtr(false)},
	}, a.toolSetPower)

	mcp.AddTool(s, &mcp.Tool{
		Name: "coral_select_preset",
		Description: "Selects the active preset by name (case-sensitive, must match an existing preset " +
			"exactly -- see coral://presets for valid names). Only takes effect if power is on; works " +
			"whether or not Coral is currently running.",
		Annotations: &mcp.ToolAnnotations{DestructiveHint: boolPtr(false)},
	}, a.toolSelectPreset)

	mcp.AddTool(s, &mcp.Tool{
		Name: "coral_set_output_device",
		Description: "Selects the audio output device by its exact Windows friendly name (see " +
			"coral://status's output_devices or coral://diagnostics's device list for valid names). " +
			"Works whether or not Coral is currently running.",
		Annotations: &mcp.ToolAnnotations{DestructiveHint: boolPtr(false)},
	}, a.toolSetOutputDevice)

	mcp.AddTool(s, &mcp.Tool{
		Name: "coral_save_preset",
		Description: "Saves the current modified settings as a new user preset and selects it. Requires " +
			"Coral to be running, power to be on, and the current preset to have unsaved changes (check " +
			"coral://presets). The name is sanitized (filesystem-reserved characters < > : \" / \\ | ? * " +
			"stripped, truncated to 64 characters) and rejected if that collides with an existing preset name.",
		Annotations: &mcp.ToolAnnotations{DestructiveHint: boolPtr(false)},
	}, a.toolSavePreset)

	mcp.AddTool(s, &mcp.Tool{
		Name: "coral_overwrite_preset",
		Description: "Overwrites the currently selected user preset with its unsaved changes. Requires " +
			"Coral to be running, power to be on, the selected preset to be a user preset (not built-in), " +
	...
	mcp.AddTool(s, &mcp.Tool{
		Name: "coral_undo_preset",
		Description: "Reverts the currently selected preset's unsaved changes. Requires Coral to be " +
			"running, power to be on, and the preset to have unsaved changes.",
		Annotations: &mcp.ToolAnnotations{DestructiveHint: boolPtr(false)},
	}, a.toolUndoPreset)

	mcp.AddTool(s, &mcp.Tool{
		Name: "coral_rename_preset",
		Description: "Renames the currently selected user preset. Requires Coral to be running, power to " +
			"be on, the selected preset to be a user preset (not built-in) with no unsaved changes. The name " +
			"is sanitized and collision-checked the same way as coral_save_preset.",
		Annotations: &mcp.ToolAnnotations{DestructiveHint: boolPtr(false)},
	}, a.toolRenamePreset)

	mcp.AddTool(s, &mcp.Tool{
		Name: "coral_delete_preset",
		Description: "Deletes the currently selected user preset. Requires Coral to be running, power to " +
...
type setPowerInput struct {
	On bool `json:"on" jsonschema:"true to turn Coral's audio processing on, false to turn it off"`
}

func (a *App) toolSetPower(ctx context.Context, _ *mcp.CallToolRequest, in setPowerInput) (*mcp.CallToolResult, applyResult, error) {
	if err := coral.Apply(ctx, a.Paths, map[string]string{"power": boolFlag(in.On)}, false); err != nil {
...
func (a *App) toolSelectPreset(ctx context.Context, _ *mcp.CallToolRequest, in selectPresetInput) (*mcp.CallToolResult, applyResult, error) {
	...
	if err := coral.Apply(ctx, a.Paths, map[string]string{"preset": in.Name}, false); err != nil {
		return nil, applyResult{}, err
	}
...
func (a *App) toolSetOutputDevice(ctx context.Context, _ *mcp.CallToolRequest, in setOutputDeviceInput) (*mcp.CallToolResult, applyResult, error) {
	...
	if err := coral.Apply(ctx, a.Paths, map[string]string{"output": in.DeviceName}, false); err != nil {
		return nil, applyResult{}, err
	}
...
func (a *App) toolSavePreset(ctx context.Context, _ *mcp.CallToolRequest, in presetNameInput) (*mcp.CallToolResult, applyResult, error) {
	...
	sanitized, err := coral.ValidatePresetName(in.Name, status)
	if err != nil {
		return nil, applyResult{}, err
	}
	if err := coral.Apply(ctx, a.Paths, map[string]string{"save_preset": sanitized}, true); err != nil {
		return nil, applyResult{}, err
	}
...
func (a *App) toolOverwritePreset(ctx context.Context, _ *mcp.CallToolRequest, _ any) (*mcp.CallToolResult, applyResult, error) {
	...
	if err := coral.Apply(ctx, a.Paths, map[string]string{"overwrite_preset": ""}, true); err != nil {
		return nil, applyResult{}, err
	}
...
func (a *App) toolUndoPreset(ctx context.Context, _ *mcp.CallToolRequest, _ any) (*mcp.CallToolResult, applyResult, error) {
	...
	if err := coral.Apply(ctx, a.Paths, map[string]string{"undo_preset": ""}, true); err != nil {
		return nil, applyResult{}, err
	}
...
func (a *App) toolRenamePreset(ctx context.Context, _ *mcp.CallToolRequest, in presetNameInput) (*mcp.CallToolResult, applyResult, error) {
	...
	if err := coral.Apply(ctx, a.Paths, map[string]string{"rename_preset": sanitized}, true); err != nil {
		return nil, applyResult{}, err
	}
...
func (a *App) toolDeletePreset(ctx context.Context, _ *mcp.CallToolRequest, _ any) (*mcp.CallToolResult, applyResult, error) {
	...
	if err := coral.Apply(ctx, a.Paths, map[string]string{"delete_preset": ""}, true); err != nil {
		return nil, applyResult{}, err
	}
	return nil, applyResult{Applied: true, Detail: fmt.Sprintf("deleted preset: %s", status.SelectedPreset)}, nil
}

// readyStatus ensures a live Coral instance is up (launching one if
...
func readyStatus(ctx context.Context, paths *coral.Paths) (*coral.Status, error) {
	if err := coral.EnsureRunning(ctx, paths); err != nil {
		return nil, err
	}
